...
use super::{v037 as r1, v037r3 as r3, v037r5 as r5};
...
pub struct LocalPlayer<'a> {
    ...
    player_v5: Option<&'a mut r5::CLocalPlayer>,
}
...
impl<'a> LocalPlayer<'a> {
    pub fn matrix(&self) -> Option<RwMatrix> {
        ...
        if let Some(player) = self.player_v5.as_ref() {
            return player.matrix();
        }
...
pub struct PlayerPool<'a> {
    ...
    pool_v5: Option<&'a mut r5::CPlayerPool>,
}

pub struct Player<'a> {
    player_v1: Option<&'a r1::CPlayerInfo>,
    player_v3: Option<&'a r3::CPlayerInfo>,
    player_v5: Option<&'a r5::CPlayerInfo>,
}
...
impl<'a> Player<'a> {
    ...
    fn new_v5(player: &'a r5::CPlayerInfo) -> Player<'a> {
        Player {
            player_v5: Some(player),
            player_v1: None,
            player_v3: None,
...
pub struct RemotePlayer<'a> {
    ...
    remote_v5: Option<&'a r5::CRemotePlayer>,
}
...
impl<'a> RemotePlayer<'a> {
    pub fn matrix(&self) -> Option<RwMatrix> {
        ...
        if let Some(remote) = self.remote_v5.as_ref() {
            return remote.matrix();
        }
...
pub fn local_player<'a>() -> Option<LocalPlayer<'a>> {
    match version() {
        ...
        Version::V037R5 => Some(LocalPlayer {
            player_v5: r5::local_player(),
            player_v1: None,
            player_v3: None
        }),
        _ => None,
...
pub fn find_player<'a>(id: i32) -> Option<Player<'a>> {
    match version() {
        ...
        Version::V037R5 => Some(Player {
            player_v1: None,
            player_v5: r5::find_player(id),
            player_v3: None,
        }),
        _ => None,
    }
...
pub fn players<'a>() -> Option<PlayersIterator<'a>> {
    match version() {
        ...
        Version::V037R5 => Some(PlayersIterator {
            players_v3: None,
            players_v1: None,
            players_v5: r5::player_pool().map(|pool| pool.m_pObject.as_mut()),
            index: 0,
        }),
...
pub struct PlayersIterator<'a> {
    ...
    players_v5: Option<&'a mut [*mut r5::CPlayerInfo]>,
    index: usize,
}
...
impl<'a> Iterator for PlayersIterator<'a> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        if let Some(players) = self.players_v5.as_ref() {
            while self.index >= 0 && self.index < 1000 {
                if let Some(player) = players.get(self.index).filter(|player| !player.is_null()) {
                    self.index += 1;
                    return Some(Player::new_v5(unsafe { &mut **player }));
                }
